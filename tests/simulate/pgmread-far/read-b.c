...
PROGMEM
#endif
const uint8_t data[] = { VAL, 2 * VAL, 7 * VAL };
...
void test (uint8_t i)
{
  if (pgm_read_byte_far (pgm_get_far_address (data[0])) != get_val (0))
    exit (__LINE__);

  uint_farptr_t pf = pgm_get_far_address (data[0]) + i * sizeof (uint8_t);
  if (pgm_read_byte_far (pf) != get_val (i))
    exit (__LINE__);
}
