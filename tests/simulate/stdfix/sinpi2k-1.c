...
void test_sinpi2k (void)
{
  float d_mi = +1000;
  float d_ma = -1000;
  uint32_t cyc = 0;

  // Test sinpi2k() for all x in [-4, 4] to have an absolute error
  ...
  for (T x = -4.0k; x <= 4.0k; x += kbits (1))
    {
      ...
      if (avrtest_cmpf (d_ma, +5.6e-5f) > 0) exit (__LINE__);
      if (avrtest_cmpf (d_mi, -5.6e-5f) < 0) exit (__LINE__);
    }
